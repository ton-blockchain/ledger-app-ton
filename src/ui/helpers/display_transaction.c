...
bool display_transaction(char *g_operation, size_t g_operation_len, char *g_amount, size_t g_amount_len, char *g_address, size_t g_address_len, char *g_payload, size_t g_payload_len) {
    // Operation
    ...
    if (G_context.tx_info.transaction.has_payload) {
        base64_encode(G_context.tx_info.transaction.payload.hash, HASH_LEN, g_payload, g_payload_len);
    } else {
