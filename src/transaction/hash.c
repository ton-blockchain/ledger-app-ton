...
void hash_Cell(struct BitString_t* bits, struct CellRef_t* refs, uint8_t refs_count, uint8_t* out) {
    cx_sha256_t state;
    cx_sha256_init(&state);
    ...
    // Finalize
    cx_hash((cx_hash_t *) &state, CX_LAST, NULL, 0, out, 64);
}
...
bool hash_tx(transaction_ctx_t *ctx) {
    ...
    // commonRef.max_depth = 0;
    // hash_Cell(&bits, NULL, 0, commonRef.hash);
    ...
    hash_Cell(&bits, NULL, 0, ctx->m_hash);
